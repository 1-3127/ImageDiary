...
v0.3의 구현과 자동 검증을 완료했으며, Windows 실사용 테스트를 준비했습니다.
...
- 종료 및 저장 시 GIF 저장 설정 팝업
- GIF 저장 설정: 저장 위치, 전체 블러, 상·하단 50px 마스킹, 반복 워터마크, 타임코드
- GIF 없이 원본 이미지만 저장 및 출력 설정 기억하기
- 완료 후 결과 폴더 열기
...
내부 원본은 `C:\temp\workdiary`에 저장됩니다. 최근 수정 세션 2개는 항상 보호되며, 나머지는 세션 생성 날짜로부터 7일이 지나면 자동 정리됩니다. GIF와 원본 이미지의 저장 위치는 종료 및 저장 후 열리는 GIF 저장 설정에서 선택합니다.
