...
class ScreenshotCaptureTests(TestCase):
    def test_saves_three_digit_sequence_png_filename(self) -> None:
        with TemporaryDirectory() as temporary_directory:
            with patch("screenshot_capture.mss.mss") as mocked_mss:
                ...
                self.assertEqual(output.name, "001.png")
    ...
    def test_does_not_overwrite_capture_with_same_sequence(self) -> None:
        with TemporaryDirectory() as temporary_directory:
            output_directory = Path(temporary_directory)
            existing = output_directory / "001.png"
